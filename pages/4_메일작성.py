...
def _label(c: dict) -> str:
    sent = " ✅" if c["id"] in sent_ids else ""
    email = c.get("email", "") or "이메일 없음"
    return f"{c['company_name']}  |  {c.get('industry', '-')}  |  {email}{sent}"

selected_idx = st.selectbox(
    "업체 선택",
    range(len(with_drafts)),
    format_func=lambda i: _label(with_drafts[i]),
)

c = with_drafts[selected_idx]
cid = c["id"]
...
with col_l:
    ...
    new_body = st.text_area("본문", value=draft.get("body", ""), height=250, key=f"body_{cid}")
...
with col_r:
    ...
    sig = st.text_area("서명", value=draft.get("signature", ""), height=130, key=f"sig_{cid}")
...
st.divider()
st.text_area("📋 복사용 전체 텍스트", value=full_mail, height=220, key=f"copy_{cid}")
